...
def return_forward_and_backward_correlation_coeffs(visual_mua, hpc_mua):
    ...
    corrcoeffs_forward = sm.tsa.stattools.ccf(visual_mua, hpc_mua)
    corrcoeffs_backward = sm.tsa.stattools.ccf(hpc_mua, visual_mua)

    return(corrcoeffs_forward, corrcoeffs_backward)


def create_x_axis_for_coeff_plot(corrcoeffs_forward, corrcoeffs_backward):
    """Create x_axis and shuffle coeffs to become -t_t_+t"""
    bin_window = 50  # 2.5 seconds either side
    x_axis = np.arange(-bin_window, bin_window, 1)  # Creates a window from -1s to 1
    minus_t_until_t_reversed = corrcoeffs_backward[1:bin_window]
    minus_t_until_t = minus_t_until_t_reversed[::-1]
    temporal_coeffs = list(minus_t_until_t) + list(corrcoeffs_forward[0:bin_window + 1])
    return(temporal_coeffs, x_axis)

def extract_arg_max(temporal_coeffs, x_axis):
