...
# import utils.helper as helper - uncomment when importing this script

#Better for frequency resolution
#Default wavelet is morelat gamma = 3, beta = 20
def cwt(filtered_signal, fs, freq_range):
    coefs_cwt, _, f_cwt, t_cwt, _ = gsp.cwt(data        = filtered_signal,
                                            fs          = fs,
                                            freq_limits = freq_range,
                                            method      = 'full',
                                            voices_per_octave = 10, #Try 10,12,14,16,32
                                            n_workers   = 8,
                                            verbose     = True)
...
#Better for temporal resolution
